...
test_name = os.path.splitext(args.target_file)[0] # 修正
test_name = test_name.split('/') # 修正
output_dir = f"{abs_path}/output_sketches/{test_name[-3]}/{test_name[-2]}/{test_name[-1]}" # 修正
...
print(f"GPU: {use_gpu}")
seeds = [2000] #list(range(0, args.num_sketches * 1000, 1000)), # 修正 seedを2000に固定
...
for seed in seeds:
    wandb_name = f"{args.num_strokes}strokes_seed{seed}"
    if multiprocess:
